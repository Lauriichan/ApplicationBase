...
public class BoxOutlineRenderer implements IRenderer {
    ...
    public final PropFloat thickness = new PropFloat(2f, 0f, Float.MAX_VALUE);

    @Override
    public void render(ImDrawList drawList, float x, float y, float width, float height, int layerOffset) {
        drawList.addRect(x + padding.left(), y + padding.top(), x + width - padding.right(),
            y + height - padding.bottom(), color.asABGR(), rounding.get(), thickness.get());
    }
