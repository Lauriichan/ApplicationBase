...
import me.lauriichan.applicationbase.app.io.serialization.SerializationException;
import me.lauriichan.applicationbase.app.io.serialization.SerializationHandler;
...
public final class IOManager {
    ...
    public IOManager(BaseApp app) {
        app.extension(IIOHandler.class, true).callInstances(handler -> {
            Class<?> next = handler.getClass().getSuperclass();
            while (next != null && next.getAnnotation(HandlerPoint.class) == null) {
                next = next.getSuperclass();
            }
            if (next == null) {
                app.logger().error("Couldn't load handler '{0}' as it has no valid handler point as super class.",
            ...
            if (map == null) {
                ...
                app.logger().error("There is already a handler of type '{0}' for value type '{1}'.", next.getName(),
