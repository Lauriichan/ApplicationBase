...
public final class ConfigManager {
    ...
    public ConfigManager(final BaseApp app) {
        if (app.conditionMap().value(ConditionConstant.DISABLE_CONFIGS)) {
            return;
        }
        app.extension(ISingleConfigExtension.class, true).callInstances(extension -> {
            configs.put(extension.getClass(), ConfigWrapper.single(app, extension));
        });
        app.extension(IMultiConfigExtension.class, true).callInstances(extension -> {
            multiConfigs.put(extension.getClass(), new MultiConfigWrapper<>(app, extension));
        });
    }
