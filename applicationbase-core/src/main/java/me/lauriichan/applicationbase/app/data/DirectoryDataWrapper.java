...
import me.lauriichan.applicationbase.app.data.IDirectoryDataExtension.FileData;
...
public final class DirectoryDataWrapper<T, D extends IDirectoryDataExtension<T>> implements IDataWrapper<T, D> {
    ...
    public static <T, D extends IDirectoryDataExtension<T>> DirectoryDataWrapper<T, D> create(final BaseApp app,
        final D extension) {
    ...
    public DirectoryDataWrapper(final BaseApp app, final D extension, final String path) {
        this.logger = app.logger();
        this.migrator = app.dataMigrator();
        this.path = path;
